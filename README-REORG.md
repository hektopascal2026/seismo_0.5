...
**Why.** Reported on the live webspace (both mothership at `/seismo/` and satellite installs): the Label tab "breaks (does not load new ones or page blank after a bit)". Tracing the controller, repository, and JS surfaced three unrelated failure modes stacked on top of each other:

1. **Rapid-click CSRF 403s.** `MagnituLabelUiController::save()` used the default `CsrfToken::verifyRequest()`, which rotates the session token on every successful POST. A labeler clicking through several cards within a few hundred ms assembled the second/third POSTs with the **pre-rotation** token; PHP serialised them on the file session lock and returned `Invalid or expired CSRF token. Reload the page.` for every POST after the first, which the JS surfaced as `alert(...)` followed by un-fading the optimistic-labeled card. After two or three of these the page looked stuck.
2. **`gatherEntries()` OOM/timeout.** `show()` eagerly loaded 4 × 280 = 1120 rows with full `feed_items.content` / `calendar_events.content` and full email `html_body` / `text_body` into PHP memory, shape-converted them, then capped to 320 for the browser. On a typical 128 MB shared-host `memory_limit` with full-article RSS rows and HTML newsletters this either ran out of memory or `max_execution_time` mid-way — PHP fatal, no `\Throwable` catch can save the page, view never renders. Reported as "page blank after a bit".
3. **No pagination past the newest 280-per-family.** Once a labeler worked through the initial 320 entries, the JS empty-state read "queue empty for this load — Reload" but reloading hit the same `OFFSET 0` window. Heavily-labeled installs ended up seeing "All caught up" forever even when older unlabeled material existed.

**What moved.**
- `src/Controller/MagnituLabelUiController.php` — `save()` now calls `CsrfToken::verifyRequest(false)` (no rotation; the session cookie is still required and still unguessable) and calls `session_write_close()` immediately after CSRF verify so consecutive label POSTs from the same browser tab don't serialise on PHP's file-based session lock. Response no longer carries a rotated `csrf` field; the JS still tolerates either shape. `show()` reads a clamped `?offset=` parameter (0-default, capped at 100000 to neutralise a malicious URL), passes it through `gatherEntries()`, and exposes `$offset` + `$nextOffset = $offset + PER_FAMILY` to the view.
- `src/Repository/MagnituExportRepository.php` — new `listFeedItemsForLabeling()`, `listEmailsForLabeling()`, `listLexItemsForLabeling()`, `listCalendarEventsForLabeling()`. Same row shape the `MagnituController::shape*()` helpers consume, but `feed_items.content` and `calendar_events.content` are omitted from the SELECT and email body columns are SUBSTRING-truncated in SQL (`SUBSTRING(..., 1, 800)` default). Each method also takes `$offset`. The Magnitu v3 sync path is unchanged — those methods still call the original `list*Since()` which select full content for the API contract.
- `views/label.php` — receives `$offset` / `$nextOffset` / a new `$pageUrl(filter, offset)` builder; renders a subtle "Showing items from offset N — Back to newest" banner when `$offset > 0`; both empty-state messages ("All caught up" at page load with nothing unlabeled, and "Done with this batch" after the in-browser queue exhausts) now link to `?action=label&type=<filter>&offset=$nextOffset` so labelers keep moving without manual URL editing.

**New wiring.** The labeling queue is now built from rows weighing ~1 KB each instead of ~50 KB each — peak memory during `gatherEntries()` drops by roughly an order of magnitude. The `shape*()` helpers still emit a `content` field (empty string on feed/calendar rows, truncated body on emails), so anything that consumed the queue's JSON shape sees the same keys; only the byte counts shrink. Pagination is by per-family OFFSET against each `list*ForLabeling()`; combined with `listLabeledKeys()`'s in-memory filter, a labeler who has worked through page 1 can click "Fetch older items" to advance to OFFSET 280, 560, … per family until older material runs out.

**Gotchas.**
- **CSRF is now a stable-per-session token** for `label_save`, deliberately. Rotation as defence-in-depth made sense for low-frequency mutating POSTs (save settings, refresh) but is incompatible with rapid mutating AJAX. Other rotating endpoints (`toggle_favourite`, `refresh_*`, `save_*`, `plugin_test`, …) are unchanged. If a future "rapid labeling" pattern appears elsewhere, copy this pattern (and document why).
- **`session_write_close()` after CSRF verify means nothing below it may touch `$_SESSION`.** The current `save()` doesn't; if someone adds a flash message later, move the close call below it.
- **Pagination is OFFSET-only.** No "you've seen everything" detection — if a label-heavy install pages past the last unlabeled row, the user just sees "All caught up" with another "fetch older" link that returns the same. Acceptable in practice (labelers stop when they're done), but if it becomes annoying we can compute a per-family `COUNT(*)` for hard-stop UX.
- **Email body truncation is in SQL, not PHP.** `SUBSTRING(..., 1, 800)` runs in MariaDB before the row leaves the DB, so the satellite cross-DB read on `getEmailTableName()` benefits too. 800 chars is enough for the JS's 220-char description trim plus headroom for `EmailListingBoilerplateStripper::strip()` to find a salutation; if a future strip pattern needs more context, raise the default in `listEmailsForLabeling($limit, $offset, $bodyChars)`.
- **`listLabeledKeys()` cap of 5000 is unchanged** in this slice. On the largest labeling installs that limit may eventually let already-labeled items reappear; if so, the next change is `ORDER BY entry_type, entry_id` + a higher cap or an `EXISTS`-based filter. Held back because the user's three reported symptoms didn't require it.
- **Uploads.** Mothership + satellite — both `src/Controller/MagnituLabelUiController.php` and `src/Repository/MagnituExportRepository.php` ship to satellites (neither is in `satellite-prune.json`), and `views/label.php` is part of the satellite UI. Schema unchanged.

---

## Lex cards — Fedlex consolidated acts get responsibility + parent-act context

**Why.** The previous Lex card slice enriched EUR-Lex with EuroVoc subjects but explicitly left Fedlex consolidated acts on the to-do list (`description = null` at `LexFedlexPlugin.php:180`). A live SPARQL probe of `https://fedlex.data.admin.ch/sparqlendpoint` over the plugin's exact scope (resource-types 21/22/29/26/27/28/8/9/10/31/32) confirmed two predicates with near-total coverage on recent SR acts: `jolux:responsibilityOf` (158/158 = 100%) gives the responsible federal office, and `jolux:classifiedByTaxonomyEntry` (153/158 ≈ 97%) gives the parent / classifying act's full title with date. Both come with `skos:prefLabel` in DE/FR/IT/EN/RM, so the configured plugin language path works directly. The taxonomy entry is the only signal that distinguishes "amendment of an existing law" (title = "Änderung vom 2. April 2026") from the underlying regime (`Lärmschutz-Verordnung vom 15. Dezember 1986 (LSV)`); without it the card is operationally useless. Companion to the EuroVoc / `description` work in commit `b197be6` — same goal (cards become more than titles), different upstream.

**What moved.**
- `src/Plugin/LexFedlex/LexFedlexPlugin.php` — primary `fetchActs()` SPARQL extended with two `OPTIONAL` blocks (`responsibilityOf` → `skos:prefLabel`, `classifiedByTaxonomyEntry` → `skos:prefLabel`), each `LANGMATCHES`-filtered to `authorityToFedlexLangPath($lang)`. `SELECT DISTINCT` is now a `SELECT … GROUP BY ?act ?title ?pubDate ?typeDoc` with `SAMPLE(?respLabelRaw)` and `GROUP_CONCAT(DISTINCT ?taxLabelRaw; SEPARATOR=" • ")` so one row per act survives the potential cross-product from multi-language or multi-taxonomy triples in the source graph. PHP row builder reads both via the `(string)($row->prop ?? '')` idiom (matches `LexEuPlugin`) and hands them to a new public static helper `composeFedlexDescription(string $title, string $resp, string $tax): ?string`. The helper joins them as `"<resp> — <tax>"` when both are present, returns the single non-empty one otherwise, and suppresses `$tax` when it is byte-identical to `$title` (avoids redundancy on freshly-consolidated acts where the two collide).

**New wiring.** Still one SPARQL call per refresh tick (unlike the EU `enrichWithEurovocSubjects()` second-query strategy) — Fedlex was happy to do the GROUP BY in-graph. `LexItemRepository::upsertBatch()` already does `description = VALUES(description)` in its `ON DUPLICATE KEY UPDATE`, so the next refresh backfills every existing Fedlex row inside the lookback window (default 90 days). `parseFedlexType()` and the `document_type` column are unchanged — the short pill labels ("Bundesgesetz", "Verordnung BR", "Amtsverordnung", …) stay categorical; the new richness lives in `description`. Consultations (`fetchConsultations()`) untouched — they already build a description from the `jolux:eventDescription` literal.

**Gotchas.**
- **Description redundancy on new acts is real and accepted.** For a brand-new consolidated act, the taxonomy entry is often a verbose-with-date copy of the title (`title = "Pflanzenschutzmittelverordnung (PSMV)"`, `tax = "Pflanzenschutzmittelverordnung vom 20. August 2025 (PSMV)"`). The byte-equal dedup in `composeFedlexDescription()` only fires on exact match, so the slight overlap renders in the card. Tolerated because the responsible office is genuinely additive in every case and the date in the taxonomy entry is informative; fuzzy dedup is more risk than reward at this size.
- **Existing recipe scores still aren't rescored.** Same constraint as the previous Lex slice — `getUnscoredLexItems()` only feeds rows without an `entry_scores` row to the recipe scorer, so legacy Fedlex items keep whatever score they had under the title-only regime. To retro-score the corpus the operator uses **Settings → Magnitu → Clear all scores** (destructive, also wipes Magnitu badges). Fresh Fedlex rows after the next refresh get the enriched description and the new recipe score in the same cron tick.
- **Multilingual coverage is symmetric.** All five Fedlex languages (DEU/FRA/ITA/ENG/ROH) have `skos:prefLabel` on the responsibility entities and on most taxonomy entries — the new `LANGMATCHES` filters pass `langPath` (2-letter "de"/"fr"/"it"/"en"/"rm") so admins running in non-German pick up office and parent-act labels in their own language without further configuration.
- **Uploads.** Mothership-only — `src/Plugin/` is wholesale removed in `satellite-prune.json`, so satellites inherit the enriched rows via the cross-DB read on `lex_items` and need no file change.
